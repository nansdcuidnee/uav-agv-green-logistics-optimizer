"""Strategy base interface for task assignment and charging selection."""
...
class BaseStrategy(ABC):
    """Base class used by all runtime strategies."""
    ...
    def select_charging_station(self, uav, environment) -> Optional[object]:
        """Select an AGV for charging the given UAV."""

    def get_idle_uavs(self, environment):
        return [uav for uav in environment.uavs if uav.is_idle()]

    def get_pending_tasks(self, environment):
        return [task for task in environment.tasks if task.status == "pending"]

    def get_available_agvs(self, environment):
