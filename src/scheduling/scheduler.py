...
class Scheduler:
    ...
    def schedule_charging(self, uavs, agvs):
        ...
        uavs_needing_charge = [
            u for u in uavs
            if u.battery < UAV_CHARGE_THRESHOLD and u.is_idle()
        ]
        ...
        for uav in uavs_needing_charge:
            if not idle_agvs:
                self.logger.warning(f"[CHARGING] No AGV available for UAV {uav.id}")
                break

            # 根据策略选择AGV
            best_agv = self.select_agv(uav, idle_agvs)

            if best_agv is not None:
                # 调度AGV前往无人机位置
                best_agv.status = "transporting"
                best_agv.charge(uav)
                ...
                self.logger.info(
                    f"[CHARGING] Schedule AGV {best_agv.id} to charge UAV {uav.id}, "
                    f"uav_battery={uav.battery}%"
                )
